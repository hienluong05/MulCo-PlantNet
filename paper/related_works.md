...
Trong lĩnh vực thị giác máy tính nông nghiệp, phần lớn các bộ dữ liệu hiện hành (như PlantDoc, PlantVillage) thường bị giới hạn ở dạng đơn phương thức, tức là chỉ chứa hình ảnh (image-only) mà thiếu đi các mô tả ngữ nghĩa đi kèm. Sự thiếu hụt này làm cản trở việc áp dụng các mô hình học đa phương thức (multimodal learning) tiên tiến. Để giải quyết nút thắt này, phương pháp Sinh mô tả hình ảnh tự động (Automated Image Description Generation - AIDG) đã ra đời [1]. Phương pháp này tận dụng sức mạnh của các Mô hình ngôn ngữ lớn đa phương thức (LMMs) để phân tích hình ảnh và tự động tạo ra các đoạn mô tả chi tiết bằng ngôn ngữ tự nhiên. 

So với phương pháp gán nhãn thủ công (manual annotation) truyền thống, AIDG mang lại những ưu điểm vượt trội [1]: chi phí cực thấp, tốc độ xử lý nhanh, và khả năng mở rộng (scalable) trên quy mô hàng vạn bức ảnh mà không bị ảnh hưởng bởi sự chủ quan hay mệt mỏi của con người. Hơn nữa, những mô tả phong phú được sinh ra sẽ cung cấp nền tảng ngữ nghĩa vững chắc để thiết lập các tập dữ liệu giả nhãn (pseudo-labeled datasets), mở ra cánh cửa cho việc chuyển đổi các bộ dữ liệu đơn phương thức thành các hệ thống chẩn đoán đa phương thức với chi phí chuẩn bị dữ liệu gần như bằng không.
